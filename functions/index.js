...
// 20세부터는 나이대 간 "상대 비율"을 실제 한국 평균 연봉 곡선(20대 약 3,000만/
// 30~49세 약 4,500만/50~64세 약 4,000만/65세+ 약 1,500만원 ≈ 1:1.5:1.33:0.5)에
// 맞췄다(2026-08-18, 사용자 지시 - "20세 이후로는 1년 평균 연봉 수준으로 배율을
// 조정해줘" → 이 비율을 그대로 절대금액에 쓰면 배포 시뮬레이션 기준 보유 현금이
// 최대 20억원대까지 치솟아 "금액은 비슷하게 두고 상대적 비율만 연봉 곡선에
// 맞춰달라"고 조정 - 그래서 절대 규모는 기존 20만/50만/40만/20만원 배율과 비슷한
// 수준을 유지하도록 기준값을 다시 스케일링했다: 20대를 20만→30만원으로 살짝
// 올리고 나머지는 그 비율(×1.5/×1.33/×0.5)로 계산). 실제 원화 금액이라 "보유
// 현금"이 음수가 되는 건 어색해서 0 밑으로는 안 내려가게 막는다(빚은 추상
// wealth 스탯에서만 표현됨).
function cashUnitForAge(age) {
  if (age < 10) return 10000;
  if (age < 20) return 30000;
  if (age < 30) return 300000;
  if (age < 50) return 450000;
