...
- 코드 구현 후 커밋·배포 전에는 반드시 검증(문법 검사, 실제 브라우저/Playwright로
  흐름 끝까지 실행, RTDB 규칙은 `--dry-run` 우선, 그 외 이 문서의 "구현 후 검증"
  관련 항목들)을 거친다.
- **검증까지 마치고 이상이 없으면, 커밋해도 되는지 따로 묻지 않고 바로 커밋 후
  `git push`까지 자동으로 진행한다(2026-09-08, 사용자가 이 레포에 명시적으로
  요청 - 기존엔 "커밋은 명시적으로 요청했을 때만" 이었으나 이 원칙을 대체함).**
  검증 중 이상이 발견되면(문법 오류, 배열 홀, id 중복, 시뮬레이션 결과가 기대와
  다름 등) 커밋하지 않고 먼저 사용자에게 알린다 - "이상 없을 때"만 자동 진행 대상.

## Firebase Functions 배포 — 함수명을 지정했다면 확인 없이 바로 진행 (2026-09-08)

- `firebase deploy --only functions:lifegame:<함수명>,functions:lifegame:<함수명>,...`
  처럼 **변경된 함수만 정확히 이름으로 나열한 배포**는, 진행해도 되는지 다시 묻지 않고
  바로 실행한다(사용자 명시 지시). 배포 전 검증(문법 검사·시뮬레이션 등)은 지금까지와
  동일하게 그대로 거친다 — 생략되는 건 "배포해도 되냐"는 확인 질문뿐이다.
- 이 자동 진행은 **개별 함수명을 나열한 경우에만** 해당한다. 함수명을 지정하지 않는
  전체 배포(`--only functions` 또는 `--only functions:lifegame`처럼 codebase 전체)는
  위 "Firebase 프로젝트 공유" 항목의 삭제 위험 경고 대상 그대로라 이 지침에서 제외 —
  그런 배포가 필요한 상황이면 여전히 먼저 확인받는다.
